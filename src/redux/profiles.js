...
export const createProfile = createAsyncThunk(
	...
	async (payload, { dispatch }) =>
	{
		try {
			...
			dispatch(setErrors({}));
			const response = await axios.post(`${process.env.REACT_APP_API_ENDPOINT}/api/profiles`, payload);
...
export const getProfiles = createAsyncThunk(
	...
	{
		try {
			const response = await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/api/profiles`, { params: payload });
...
export const getProfilesOptions = createAsyncThunk(
	"calls/getProfilesOptions",
	async ({ type }) =>
	{
		try {
			const response = await axios.get(`${baseURL}/profile/status/list/${type}`);
