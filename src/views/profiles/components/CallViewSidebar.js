...
export default ({ call, open, toggleSidebar }) => {
    ...
    return (
        ...
        >
            ...
            {
                call.tags.map(tag => {
                    return <Badge style={{ marginRight: "5px" }} key={`tag-${tag.key}`} color='light-primary'>{tag.label}</Badge>
                })
            }
            ...
            <p>{call.updated_at ? moment(call.updated_at).format("YYYY-MM-DD HH:mm:ss") : "-"}</p>
        </Sidebar>
    )
}