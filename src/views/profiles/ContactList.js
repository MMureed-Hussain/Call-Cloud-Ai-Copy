...
export default () =>
{
    ...
    const [data, setData] = useState({ sort: 'desc', orderby: 'created_at', per_page: per_page, search: '', call_profile_id: params.id });
        ...
        <>
            <Card>
                <CardHeader className="py-1">
                    ...
                    <Button onClick={() => CreateUpdateContactSidebarRef.current.handleShow()}> Add Contact</Button>
                </CardHeader>
                <div className="p-1">
                    <Row>
                        <Col lg="10">
                        ...
                        <Col lg="2">
                            <Input className="mb-2" name="search" value={data.search} onChange={e => handleChange(e)} placeholder="Search..." style={{ maxHeight: '50px' }} />
                        </Col>
                    </Row>
                </div>
                <div className="react-dataTable">
                    <Table responsive>
                        ...
                        <tbody>
                                            ...
                                            <div className="d-flex">
                                                <UncontrolledDropdown>
                                                    ...
                                                    <DropdownMenu container={"body"} end>
                                                        <DropdownItem onClick={() => CreateUpdateContactSidebarRef.current.handleShow(row)} >
                                                            <Edit size={15} />
                            ...
                            {(contacts && !Boolean(contacts.data.length)) && <tr> <td className="text-center" colSpan={7}>There are no records to display</td></tr>}
                        </tbody>
                    </Table>
                </div>
                <Row>
                    <Col className="small">
                        {(contacts && Boolean(contacts.data.length)) && <div className="my-2 ms-1"> Showing {contacts.from} to {contacts.to} of {contacts.total} entries</div>}
                    </Col>
                    <Col>

                        {(contacts && Boolean(contacts.data.length)) && <ReactPaginate
                            previousLabel={""}
                            nextLabel={""}
                            pageCount={contacts && contacts.last_page !== 0 ? contacts.last_page : 0}
                            activeClassName="active"
                            onPageChange={({ selected }) => loadList({ page: selected + 1 })}
