...
import { setReloadTable } from "../../../redux/followups";

import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faVideo, faPhone } from "@fortawesome/free-solid-svg-icons";
import { getTimezones } from "@store/auth";
...
const FollowUpSidebar = forwardRef((props, ref) =>
{
	...
	const timezoneList = useSelector((state) => state.auth.timezonesList);
		...
		<Sidebar
			...
			<Form onSubmit={handleSubmit}>
				...
				{!timezone &&
					...
					<div className="mb-1">
						<Label className="form-label" for="timezoneInput"> Profile's Timezone </Label>
						<Select
							required
							className="mb-2"
							classNamePrefix="select"
							options={[{ value: null, label: 'None Selected' }, ...timezoneList]}
							onChange={(e) => handleSelectChange(e, 'timezone')}
							placeholder="select a timezone"
							value={handleSelected(timezoneList, timezone)}
						/>
