...
import { getCallsList, leadProfileStatusList, setErrors, getRecordingByWorkspace } from "../../redux/calls";
                            ...
                            <tbody>
                                {recordings.map((row, ind) =>
                                    <tr key={ind} className="text-center">
                                        <td>
                                            <UserInfo name={`${row.sender.first_name} ${row.sender.last_name}`} email={row.sender.email} />
                                        </td>
                                        <td>
            ...
            <Card>
                ...
                <div className="react-dataTable">
                    <Table responsive>
                        ...
                        <tbody>
                            {calls.data && calls.data.map((row, ind) =>
                                <tr key={ind} >
                                    <td>
                                        <UserInfo
                                    ...
                                    <td>
                                        <Badge color="primary" className="bg-light-primary">{row.call_status && row.call_status.name.toUpperCase()}</Badge>
                                    </td>
                                    <td>
                                        <div>{row.callprofile && row.callprofile.name}</div>
                                        {(row.callprofile && row.callprofile.lead_status) &&
                                            <Badge color="warning" className="bg-light-warning">{row.callprofile.lead_status.name.toUpperCase()}</Badge>
                                        }
                                    </td>
                                    <td>
                                        {row.callprofile &&
                            ...
                            {(calls && !Boolean(calls.data.length)) && <tr><td colSpan={6} className="text-center">No data Found!</td></tr>}
                        </tbody>
                    </Table>
                </div>
                <Row>
                    <Col className="small">
                        {calls.data && <div className="my-2 ms-1"> Showing {calls.from} to {calls.to} of {calls.total} entries</div>}
                    </Col>
                    <Col>

                        {(calls && Boolean(calls.data.length)) && <ReactPaginate
                            previousLabel={""}
                            nextLabel={""}
                            pageCount={calls && calls.last_page !== 0 ? calls.last_page : 0}
                            activeClassName="active"
                            onPageChange={({ selected }) => loadData({ page: selected + 1 })}
